...
from renewable_service_dataclasses import DayAheadRenewablesOutput, RenewableStateOutput
...
class RenewableService(RenewableServiceBase): 
    ...
    def init_calculation_service(self, energy_system: EnergySystem):
        ...
        self._curtailment_limit = {}  # esdl_id -> last curtailment limit from Network Balancer
        ...
        for esdl_id in self.simulator_configuration.esdl_ids:
            ...
            # Derive the effective collector area from the nameplate rating so that
            # nameplate power is the single source of truth for generation. The
            # fixed ESDL surfaceArea previously made the PV capacity sweep (Axis 3)
            # a no-op: output was driven by the constant area and only the nameplate
            # cap (which rarely binds) varied with capacity, so every non-zero size
            # produced the same yield. With area = P_nom / (G_STC * panel_eff *
            # inverter_eff), generation scales linearly with the swept nameplate.
            G_STC = 1000.0  # W/m^2 standard-test irradiance
            surface_area = capacity_w / (G_STC * max(1e-6, panel_eff * inv_eff))
    ...
    def calculate_production_w(self, esdl_id, irradiance, t):
        ...
        # Atmospheric variance (deterministic based on time)
        rng = random.Random(int(t.timestamp() // 900))
        return max(0.0, capped_w * rng.uniform(0.98, 1.02))
    ...
    def day_ahead_renewables(self, param_dict, simulation_time, time_step_number, esdl_id, energy_system):
        ...
        for i in range(96):
            ...
            forecast.append(self.calculate_production_w(esdl_id, irrad, t))
